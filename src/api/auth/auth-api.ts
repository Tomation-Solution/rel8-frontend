...
export const verifyUserMembership = async (
  data: VerifyMembershipForm
): Promise<VerifiedMembershipResponse> => {
...
export const createMember = async (data: any) => {
  const response = await apiPublic.post(
    `/auth/ManageMemberValidation/create_member/`,
    data
  );
  return response.data;
};
...
export const memberLogin = async (data: {
  ...
  const response = await apiPublic.post(`api/members/login`, data);
  return response.data;
};

export const requestPassword = async (data: { email: string }) => {
  const response = await apiPublic.post(
    "/user/forgot-password/request_password_change/",
    data
  );
  return response.data;
};
