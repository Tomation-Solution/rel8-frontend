...
export const fetchPublicationsComments = async (id: string | null) => {
  if (id) {
    const response = await apiTenant.get(
      `/publication/publicationview__comment/?publication_id=${id}`
    );
    return response.data;
  }
};
...
export const postPublicationComment = async (
  comment: string,
  newsId: number
) => {
  const requestBody = {
  ...
  const response = await apiTenant.post(
    `/publication/publicationview__comment/`,
    requestBody
  );
  return response.data;
};
...
export const deletePublicationComment = async (commentId: number) => {
  const response = await apiTenant.delete(
    `/publication/publicationview__comment/${commentId}/`
  );
  return response.data;
};
