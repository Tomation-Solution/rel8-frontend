...
const Toggle = ({ checked, onChange, label, description, disabled = false, className = "" }: ToggleProps) => {
  const control = (
    <button
      ...
      className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${checked ? "bg-org-primary" : "bg-neutral-2"} ${disabled || !onChange ? "opacity-60 cursor-not-allowed" : ""}`}
    >
