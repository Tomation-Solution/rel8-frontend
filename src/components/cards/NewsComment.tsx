...
export default function NewsComment({ newsId }: NewsCommentProps) {
  ...
  const { data, isLoading, isError } = useQuery(["news", newsId], () =>
    fetchNewsComments(newsId?.toString() || null)
  );
  const comments: Comment[] = data?.data || [];

  const postMutation = useMutation(
    (newComment: { comment: string; newsId: number }) =>
      postNewsComment(newComment.comment, newComment.newsId),
    {
      onSuccess: () => {
  ...
  const deleteMutation = useMutation(
    (commentId: number) => deleteNewsComment(commentId),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(["news", newsId]);
      },
      ...
      <div className="mt-10">
        ...
        <div className="h-[100px] flex items-center gap-2">
          <div className="rounded-full">
            <Avatar
              imageUrl={user?.profile_image ? user.profile_image : avatarImage}
            />
          </div>
