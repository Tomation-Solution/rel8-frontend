...
import { fetchPublicationsComments, postPublicationComment, deletePublicationComment } from "../../api/publications/publications-api";

// Define the component with props type
export default function PublicationComment({ newsId }: NewsCommentProps) {
    ...
    const deleteMutation = useMutation((commentId: number) => deletePublicationComment(commentId), {
        onSuccess: () => {
            queryClient.invalidateQueries(['publication', newsId]);
        },
    });
