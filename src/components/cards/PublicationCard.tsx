...
const PublicationCard = ({
  ...
  const formattedDate = new Date(
    (publicationItem as any).updated_at ||
      (publicationItem as any).updatedAt ||
      (publicationItem as any).createdAt ||
      ""
  ).toLocaleDateString();
  const content =
    (publicationItem as any).content || (publicationItem as any).body;
  return (
    <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow">
      <img
        className="rounded-t-lg w-full md:h-[180px] h-[170px] object-contain"
        src={
          (publicationItem as any).fileUrl || (publicationItem as any).bannerUrl
        }
        alt=""
      />
      <div className="md:p-5 p-2">
        <Link
          to={`/${linkTo}/${
            (publicationItem as any).id || (publicationItem as any)._id
          }/`}
        >
          <h5 className="mb-2 md:text-[15px] text-sm font-bold tracking-tight text-textColor line-clamp-1 dark:text-white">
            {(publicationItem as any).title || (publicationItem as any).topic}
          </h5>
        </Link>
        <p className="mb-2 text-[10px] line-clamp-1">{formattedDate}</p>
        <p className="mb-3 font-normal text-textColor line-clamp-2 text-sm">
