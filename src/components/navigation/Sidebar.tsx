...
const Sidebar = ({ isMobileSidebarOpen, setIsMobileSidebarOpen }: Props) => {
  ...
  const [sideBarData, setSideBarData] =
    useState<SideBarLinkType[]>(initialSideBarData);

  const { data } = useQuery("committees", fetchAllCommittees);

  console.log(data, "The Committee");
  ...
  useEffect(() => {
    if (data && data.data) {
      if (data.data.length === 0) {
        ...
        const committeeItems: SubMenuItem[] = data.data.map(
          (committee: Committee) => ({
            name: committee.name,
      ...
      {isMobileSidebarOpen && (
        <div
          ...
          className={`lg:hidden fixed top-0 w-full min-h-screen h-full bg-black/70 transition-transform z-10 ${
            isMobileSidebarOpen
              ? "translate-x-0 ease-in duration-900"
              : "-translate-x-full ease-out duration-900"
          }`}
        >
          <nav
