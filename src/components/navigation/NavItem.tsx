...
const NavItem = ({
  ...
  return (
    <>
      ...
      >
        <div
          className={`flex my-1 items-center justify-between gap-3 group text-[15px] font-[0.875rem] text-[#6C7383] p-3 mx-4 rounded-lg group ${
            location.pathname === item.path
              ? "bg-activeLink text-white"
              : "hover:bg-activeLink hover:text-white"
          }`}
        >
        ...
        <div className="ml-6 mt-2 space-y-2">
          {item.subMenu.map((subItem, index) => (
            <div
              key={index}
              onClick={() =>
                handleSubMenuClick(subItem.path, subItem.isMessage)
              }
