...
export const setRel8UserRegistrationData = (
  data: UserRegistrationParticluarsDataType
) => {
...
export const FetchName = (user: any) => {
  ...
  const memberInfoFullName = user.member_info?.find(
    (info: any) => info.name === "fullname"
  )?.value;
...
export const FetchNameByMemberInfo = (
  member_info: MemberType["member_info"]
): string => {
  // @ts-ignore
  const name: any = member_info.find((d) => {
    return (
      d.name.toLocaleLowerCase() == "name" ||
      d.name.toLocaleLowerCase() == "first" ||
      d.name.toLocaleLowerCase() == "first name" ||
      d.name.toLocaleLowerCase() == "surname"
    );
  })["value"];
  if (typeof name === "string") {
    return name;
