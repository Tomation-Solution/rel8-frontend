...
const MessageBubble = ({ by, sender, message, time }: Props) => (
  <div className={`flex flex-col w-fit max-w-[75%] sm:max-w-[55%] m-2 animate-fade-in ${sender ? "ml-auto items-end" : "mr-auto items-start"}`}>
    {!sender && <small className="text-xs text-org-primary font-medium mb-1">{by || "Anonymous"}</small>}
    <p className={`text-sm rounded-lg px-3 py-2 whitespace-pre-line animate-slide-up ${sender ? "bg-org-primary text-white" : "bg-white text-ink border border-hairline"}`}>{message}</p>
