...
import { fetchAllUserNews, likeDislikeNews, dislikeNews } from "../../../api/news/news-api";
...
const NewsDetailPage = () => {
  ...
  const likeMutation = useMutation((newsId: string) => likeDislikeNews(newsId), {
    onSuccess: response => {
  ...
  const dislikeMutation = useMutation((newsId: string) => dislikeNews(newsId), {
    onSuccess: response => {
        ...
        <div className="grid md:grid-cols-4 md:gap-10 gap-[50px] px-5">
          <div className="col-span-3 min-w-0 overflow-hidden">
            ...
            <div className="col-span-1 mt-6">
              ...
              <div
                className="[&_img]:max-w-full [&_img]:h-auto [&_iframe]:max-w-full [&_iframe]:w-full [&_table]:w-full [&_table]:block [&_table]:overflow-x-auto [&_pre]:overflow-x-auto [&_pre]:whitespace-pre-wrap [&_p]:break-words [&_a]:break-all"
                dangerouslySetInnerHTML={{ __html: `${newsItem?.content || newsItem?.body}` }}
              />
            </div>
            <div className="flex gap-2 mt-4 items-center">
              <button className={`px-2 py-1 rounded-md flex items-center gap-1 ${hasLiked ? "bg-green-500 text-white" : "bg-gray-200"}`} onClick={handleLike}>
                <AiOutlineLike />
                <span>{newsItem?.likes?.length || 0}</span>
              </button>
              <button className={`px-2 py-1 rounded-md flex items-center gap-1 ${hasDisliked ? "bg-red-500 text-white" : "bg-gray-200"}`} onClick={handleDislike}>
                <AiOutlineDislike />
                <span>{newsItem?.dislikes?.length || 0}</span>
              </button>
            </div>
            {(newsItem?.attachmentUrls && newsItem.attachmentUrls.length > 0) ||
              (newsItem?.bannerUrl && (
                <div>
                  <button className="px-3 w-[240px] my-4 py-2 bg-org-primary text-white border border-white h-[40px] rounded-md hover:text-org-primary-blue hover:border-primary-blue" onClick={downloadPublication}>
