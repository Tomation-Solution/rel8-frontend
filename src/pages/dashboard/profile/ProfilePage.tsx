      ...
      <section className="col-span-5 xl:col-span-3 ">
        <div className="grid grid-cols-5 h-[200px] gap-4 text-sm">
          ...
          <div className="col-span-2 font-light h-full flex justify-center flex-col gap-y-3">
            <div className="">
              <h4 className="font-semibold text-xl capitalize my-1">
                {userProfile?.data?.data[0]?.more_info[0]?.value}
              </h4>
              {/* <p className="text-sm my-1" >Employed full time since 2005</p> */}
              <p className="text-sm my-1">
                {userProfile?.data?.data[0]?.more_info[1]?.value}
              </p>
              <p className="text-sm font-medium my-1">
                Matriculation No. :{" "}
                <span className="font-light">
                  {" "}
                  {userProfile?.data?.data[0]?.more_info[5]?.value}
                </span>
              </p>
              <p className="text-sm font-medium my-1">
                Chapter :{" "}
                <span className="font-light">{user?.chapter.name}</span>
              </p>
            </div>
        ...
        <div className="flex flex-col gap-y-4 my-4 text-sm">
          <div className="flex items-center gap-2">
            <img
              ...
              alt=""
            />
            <span className="flex-1 bg-neutral-3 p-[10px] rounded-md px-3">
              {userProfile?.data?.data[0].more_info[2].value === null
                ? "Not Available"
                : userProfile?.data?.data[0].more_info[2].value}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <img
          ...
          <div className="flex flex-col gap-2 mt-7">
            ...
            <span className="flex-1 bg-neutral-3 p-[10px] rounded-md text-txt px-3">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Et lacus
              lacus, proin proin egestas. Augue scelerisque pellentesque nullam
              montes, pretium. Nisl, in netus Et lacus lacus, proin proin
              egestas. Augue. Lorem ipsum dolor sit amet, consectetur adipiscing
              elit. Et lacus lacus, proin proin egestas. Augue scelerisque
              pellentesque nullam montes, pretium. Nisl, in netus Et lacus
              lacus, proin proin egestas. AugueLorem ipsum dolor sit amet,
              consectetur adipiscing elit. Et lacus lacus, proin proin egestas.
              Augue scelerisque pellentesque nullam montes, pretium. Nisl, in
              netus Et lacus lacus, proin proin egestas. Augue.. Read more
            </span>
          </div>
        </div>
      </section>
