...
const ChatPage = () => {
  ...
  const { data, isLoading } = useQuery(queryKey, () => {
    if (currentChatType.type === "general") {
      ...
      if (logged_in_user) {
        const room_name =
          logged_in_user?._id > reciver_id
            ? `${logged_in_user?._id}and${reciver_id}`
            : `${reciver_id}and${logged_in_user?._id}`;
  ...
  const currentItems = allChatUsers?.data?.data?.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 my-4">
        <div className="flex flex-col space-y-2 ">
          ...
          >
            ...
            {currentItems
              ?.filter((d: any) => d.user !== logged_in_user?.user_id)
