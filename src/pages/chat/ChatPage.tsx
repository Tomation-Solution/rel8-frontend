...
const ChatPage = () => {
  ...
  const { data, isLoading } = useQuery(queryKey, () => {
    if (currentChatType.type === "general") {
      ...
      if (logged_in_user) {
        const room_name =
          logged_in_user?.user_id > reciver_id
            ? `${logged_in_user?.user_id}and${reciver_id}`
            : `${reciver_id}and${logged_in_user?.user_id}`;
  ...
  // console.log(allChatUsers, "all chat users");
  const filteredItems = currentItems
  ?.filter((d: any) => true || d.user !== logged_in_user?.user_id)
  console.log(filteredItems, "current items");
  return (
    <main className="">
              ...
              ?.map((chatUser: any, index: number) => (
                <div
                  ...
                  onClick={() => {
                    ...
                    setCurrentChatType({
                      type: "single-chat",
                      display: chatUser.name , //FetchName(chatUser),
                ...
                >
                  ...
                  <div className="flex-1">
                    <h4
                      className={`font-medium text-capitalize 
        `}
                    ...
                    <small
                      className={`text-capitalize text-textColor text-xs font-light `}
                      //  ${selected ? "text-white" : "text-textColor"}
                    >
                      {/* {lastMessage} */}
                    </small>
